...
一个具备长期记忆、语音交互与工作能力的 Windows 桌面"搭子"（companion）——Q 版精灵形象、会记住你的生活轨迹、能听能说、能帮你看屏幕干活陪你下棋的赛博伙伴。

> 特性
> - **Q 版精灵搭子**：透明置顶窗口，10 种表情、待机眨眼、说话口型、点击分区反馈（呆毛/头/身/尾巴各有专属反应）、拖拽跑动、主动搭话
> - **语音交互**：按键说话（本地 SenseVoice 识别）+ 回复发声（GPT-SoVITS 克隆音色 / EdgeTTS 兜底）+ 免提唤醒词 + 说话即打断
> - **伙伴游戏**：说"来一局五子棋"自动开局，她边下边用人设口吻点评局势
> - **长期记忆**（融合"检索精度 × 在场感"）：
...
>   - **语义检索**（可选）——`uv sync --extra semantic` 启用 bge-small-zh 本地向量（onnx，无 torch），混合余弦+词面打分；未安装时自动降级词面检索
...
```bash
uv sync --extra voice --extra semantic  # 安装（含语音与语义检索，Python 3.12+，uv 管理）
